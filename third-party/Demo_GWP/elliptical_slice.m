function [xx, cur_log_like] = elliptical_slice(xx, prior, lr, log_like_fn, cur_log_like, angle_range, varargin)
...
if (nargin < 5) || isempty(cur_log_like)
    cur_log_like = log_like_fn(S,v,V);
end
...
if numel(prior) == D
    ...
    nu = reshape(prior'*randn(D, 1), size(x));
end
hh = lr*log(rand) + cur_log_like;
...
while true
    ...
    inicio=1;
    fin=Ndatos;
    xx_prop = real(x*cos(phi) + nu*sin(phi));
        
        for i=1:v
            for j=1:fil
            u{i,j}=xx_prop(inicio:fin);
            inicio=inicio+Ndatos;
            fin=Ndatos+fin;
            end
        end
  
        if(strcmp(func2str(log_like_fn),'log_lik_wishart'))
            [V]=GWP_construct(u,L,v);
            cur_log_like = log_like_fn(S,v,V);
        elseif(strcmp(func2str(log_like_fn),'log_lik_frob'))
            [V]=GWP_construct(u,L,v);
            cur_log_like = log_like_fn(S,V);
        else
            [V]=GWP_scalar(u,L,v);
            estruc.M=V;estruc.Dat=S;estruc.Ndatos=Ndatos;
            cur_log_like = log_like_fn(estruc);    
        end
    if cur_log_like > hh
